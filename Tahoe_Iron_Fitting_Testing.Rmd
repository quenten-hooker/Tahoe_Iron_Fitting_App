...
Data_Engage_Quad_Carlsbad_Iron_Select <- Data_Engage_Quad_Carlsbad_Iron_Select %>% filter(SmashFactor > 1.2, ImpactHeadSpeedMph < 120, ImpactHeadSpeedMph > 50) 
...
mod_regress <-lmer(ImpactHeadSpeedMph ~
           # Fixed-effects
           1 + BallSpeedMph + BallLaunchAngleDeg + BallSideAngleDeg + BallBackSpinRpm + BallSideSpinRpm + ClubFamilyName + ImpactHorizLocInch + ImpactVertLocInch +
           # Random-effects
           (1 + BallSpeedMph + BallLaunchAngleDeg + BallSideAngleDeg + BallBackSpinRpm + BallSideSpinRpm | FittingId), data = train)
...
data <- Data_Engage_Quad_Carlsbad_Iron_Select %>% select(ImpactHeadSpeedMph, BallSpeedMph, BallLaunchAngleDeg, BallSideAngleDeg, BallBackSpinRpm, BallSideSpinRpm)
...
n <- names(train_)
f <- as.formula(paste("ImpactHeadSpeedMph ~", paste(n[!n %in% "ImpactHeadSpeedMph"], collapse = " + ")))
nn <- neuralnet(f,data=train_,hidden=c(5,3),linear.output=T)
